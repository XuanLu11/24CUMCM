'''这是用于计算第5题的程序,使用了program4_1.py得到的结果'''
#调用库和方法
import numpy as np
import math
import pandas as pd
...
def getNextVelocity(k1:float,k2:float,k:float,v:float)->float:
    ...
    return v*c1/c2
type=np.zeros((224,DT_NUM+1))
#0:在盘入螺旋线上,1:在大圆弧上,2:在小圆弧上,3:在盘出螺旋线上
parameter=np.zeros((224,DT_NUM+1))
#点在曲线上的位置所对应的参数
k=np.zeros((224,DT_NUM+1))
v=np.zeros((224,DT_NUM+1))
x=np.zeros((224,DT_NUM+1))
y=np.zeros((224,DT_NUM+1))
for i in range(DT_NUM+1):
    t=START_TIME+i*DT
    print('now t:'+'%.2f'%(t))
...
for i in range(DT_NUM+1): 
    #计算不同位置在不同时间下的速度(velocity)
    t=START_TIME+i*DT
    print('v calc,now t:'+'%.2f'%(t))
    v[0,i]=HEAD_VECOCITY
...
if type[223,0]!=3:
    for i in range(DT_NUM+1):
        t=START_TIME+i*DT
        if not flag1 and type[223,i]==1:
            flag1=True
            print('龙尾(后)进入调头空间的时间(s):'+'%.2f'%(t))
        if not flag2 and type[223,i]==2:
            flag2=True
            print('龙尾(后)进入小圆弧的时间(s):'+'%.2f'%(t))
        if not flag3 and type[223,i]==3:
            print('龙尾(后)离开调头空间的时间(s):'+'%.2f'%(t))
            break
#龙尾(后)进入调头空间的时间(s):370.00
#龙尾(后)进入小圆弧的时间(s):379.00
#龙尾(后)离开调头空间的时间(s):383.30
v_max=np.zeros(DT_NUM+1)
for i in range(DT_NUM+1):
    v_max[i]=max(v[:,i])
index=np.argmax(v_max)
v0=v_max[index]
print('龙头为1m/s时各部分的最大速度(m/s):'+'%.6f'%(v0))
#START_TIME=0,END_TIME=500
#计算所得:龙头速度为1m/s时,步长为0.1s,各部分的最大速度为1.603835m/s
#龙头速度为1m/s时,步长为0.1s,各部分的最大速度为1.604793m/s
print('龙头的最大速度(m/s):'+'%.6f'%(2/v0))
#也就是说,龙头的最大速度为1.246267m/s
...
x0=[i*0.1 for i in range(5001)]
plt.plot(x0,v_max,color='#0000FF',label='最大速度曲线')
plt.xlabel('时间(s)')
plt.ylabel('最大速度(m/s)')
plt.title('龙头速度为1m/s时各部分的最大速度(步长为0.1s)')
plt.legend()
plt.grid(True)
plt.show()

x0=[i*0.1 for i in range(201)]#0~20s
plt.plot(x0,v_max[:201],color='#0000FF',label='最大速度曲线')
plt.xlabel('时间(s)')
plt.ylabel('最大速度(m/s)')
plt.title('龙头速度为1m/s时各部分的最大速度(步长为0.1s)')
plt.legend()
...
x0=[i*0.1 for i in range(3600,3901)]#360~390s
plt.plot(x0,v_max[3600:3901],color='#0000FF',label='最大速度曲线')
plt.xlabel('时间(s)')
plt.ylabel('最大速度(m/s)')
plt.title('龙头速度为1m/s时各部分的最大速度(步长为0.1s)')
plt.legend()
...
x0=[i*0.1 for i in range(3830,5001)]#383~500s
plt.plot(x0,v_max[3830:],color='#0000FF',label='最大速度曲线')
plt.xlabel('时间(s)')
plt.ylabel('最大速度(m/s)')
plt.title('龙头速度为1m/s时各部分的最大速度(步长为0.1s)')
plt.legend()
